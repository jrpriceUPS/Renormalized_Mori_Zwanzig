...
for j = 1:length(alpha_list)
    ...
    [c_find,laws_find,r_find] = renormalize_frac(u_array,N_list,t_array,1/2,1);
    coefficients(j,:,:,:) = c_find;
    laws(j,:,:,:) = laws_find;
    r(j,:,:) = r_find;
    save('diff_IC_coeffs_half','coefficients')
    save('diff_IC_laws_half','laws')
    save('diff_IC_r_half','r')
...
for i = 1:length(alpha_list)
    ...
    for j = 1:4
        ...
        saveas(gcf,sprintf('coeff_plot_smaller_IC_ROM_half_%i',j),'png')
    end
end
