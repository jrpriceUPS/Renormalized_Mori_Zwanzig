...
for j = 1:length(alpha_list)
    ...
    for i = 1:length(N_list)
        ...
        if exist([sprintf('u_array4_%i_%i_',N,end_time) namelist{j} '.mat'],'file') == 2
            
            load([sprintf('u_array4_%i_%i_',N,end_time) namelist{j} '.mat'])
            load([sprintf('t4_%i_%i_',N,end_time) namelist{j} '.mat'])
            ...
            save([sprintf('t4_%i_%i_',N,end_time) namelist{j} '.mat'],'t4');
            save([sprintf('u_array4_%i_%i_',N,end_time) namelist{j} '.mat'],'u_array4');
            
        end
        
        % plot the energy in some modes
        if exist([sprintf('energy_%i_%i_',N,end_time) namelist{j}],'file') == 2
            load([sprintf('energy_%i_%i_',N,end_time) namelist{j}])
            load([sprintf('t4_%i_%i_',N,end_time) namelist{j} '.mat'])
        else
            energy = get_3D_energy(u_array4,N);
            save([sprintf('energy_%i_%i_',N,end_time) namelist{j} '.mat'],'energy');
        end
        figure(1)
        hold on
        ...
        saveas(gcf,[sprintf('energy_%i_%i_',N,end_time) namelist{j}],filetype)
...
%         saveas(gcf,[sprintf('energy_mult_slopes_%i_',N) namelist{j}],filetype)
        ...
        % plot the enstrophy
        if exist([sprintf('enstrophy_%i_%i_',N,end_time) namelist{j} '.mat'],'file') == 2
            load([sprintf('enstrophy_%i_%i_',N,end_time) namelist{j} '.mat'])
        else
            ens = enstrophy(u_array4);
            save([sprintf('enstrophy_%i_%i_',N,end_time) namelist{j} '.mat'],'ens');
        end
        [m,peak_time] = max(ens);
        ens_max(i) = m;
        ...
        saveas(gcf,[sprintf('enstrophy_mult_%i_',N) namelist{j}],filetype)
        ...
        xlabel('time','fontsize',16)
        ylabel('enstrophy','fontsize',16)
        saveas(gcf,[sprintf('enstrophy_mult_trim_%i_',N) namelist{j}],filetype)
        
        
        % plot the vorticity
        if exist([sprintf('vorticity_%i_%i_',N,end_time) namelist{j} '.mat'],'file') == 2
            load([sprintf('vorticity_%i_%i_',N,end_time) namelist{j} '.mat'])
        else
            [~,vort2] = vorticity(u_array4);
            save([sprintf('vorticity_%i_%i_',N,end_time) namelist{j} '.mat'],'vort2');
        end
        ...
        saveas(gcf,[sprintf('vorticity_mult_%i_',N) namelist{j}],filetype)
        ...
        xlabel('time','fontsize',16)
        ylabel('maximal vorticity','fontsize',16)
        saveas(gcf,[sprintf('vorticity_mult_trim_%i_',N) namelist{j}],filetype)
