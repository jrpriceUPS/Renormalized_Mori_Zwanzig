function nonlin4 = t4model_term_complete_KdV(alpha,F_modes,G_modes,k,epsilon,u_full,uu,uu_star,uk3,uk6,A,A_star,B,B_star,C,C_star,D,D_star,E,E_star,F,F_star)
%
...
internal1 = convolution_sum_KdV(u_full,-epsilon^4*uk6+1j*epsilon^2*(A+A_star)...
                                   +2*B - 4*C - 4*B_star + 2*C_star,alpha);
internal1(F_modes) = 0;

internal2 = 1j*epsilon^2*k.^3.*convolution_sum_KdV(u_full,-3*epsilon^4*uk6 ...
                                                      +1j*epsilon^2*(3*A+A_star) ...
                                                      -2*(-3*B+5*C) ...
                                                      +2*(-3*B_star+C_star),alpha);
internal2(F_modes) = 0;                      
                          
auxiliary1 = 2*convolution_sum_KdV(u_full,epsilon^4*uk6-1j*epsilon^2*(A+3*A_star)...
                                      +2*(3*C-B)+2*(5*B_star-3*C_star),alpha);
auxiliary1(G_modes) = 0;
auxiliary2 = 2*convolution_sum_KdV(u_full,-3*epsilon^4*uk6+1j*epsilon^2*(3*A+A_star)...
                                      +2*(3*B-5*C)+2*(-3*B_star+C_star),alpha);
auxiliary2(F_modes) = 0;
internal3 = convolution_sum_KdV(u_full,1j*k.^3.*uk6*epsilon^6 ...
...
internal4 = convolution_sum_KdV(1j*epsilon^2*uk3+uu,3*epsilon^4*uk6 - 1j*epsilon^2*(3*A+A_star) ...
                                                  +2*(-3*B+5*C)+2*(3*B_star-C_star),alpha);
internal4(F_modes) = 0;

internal5 = convolution_sum_KdV(uu_star,-epsilon^4*uk6 ...
                                    +1j*epsilon^2*(A+3*A_star) ...
                                    +2*B - 6*C - 10*B_star + 6*C_star,alpha);
internal5(F_modes) = 0;

nonlin4 = 2*convolution_sum_KdV(u_full,-1j*epsilon^6*k.^6.*A_star...
                                   ...
                                   +2*internal5,alpha) ...
         +8*convolution_sum_KdV(uu_star,-k.^3*epsilon^4.*A_star ...
                                    +2*1j*epsilon^2*k.^3.*(-2*B_star+C_star)...
                                    +2*internal1...
                                    +2*(E_star-F_star)...
                                    +2*D_star,alpha)...
         -48*convolution_sum_KdV(B_star,1j*epsilon^2*A_star + 2*C_star,alpha)...
         +6*convolution_sum_KdV(1j*epsilon^2*A_star+2*(B_star+C_star),...
