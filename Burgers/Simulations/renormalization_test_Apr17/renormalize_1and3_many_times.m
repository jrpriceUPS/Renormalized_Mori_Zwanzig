function [c1,c3] = renormalize_1and3_many_times(alpha,N_list,u_list,t_list,exact_derivative,time)
...
c3 = zeros(2,length(N_list),length(t_list));
...
%     subplot(2,1,1)
%     hold off
%     plot(log(N_list),log(squeeze(c1(1,:,j))),'b.','markersize',20)
%     hold on
%     plot(log(N_list),log(squeeze(c3(1,:,j))),'k.','markersize',20)
%     xlabel('log(N)','fontsize',16)
%     ylabel('log(a)','fontsize',16)
%     title(sprintf('t-model coefficient, t = %i',t_list(j)),'fontsize',16)
%     legend('Only fit t-model','fit t-model and t^3-model')
%     
%     subplot(2,1,2)
%     hold off
%     plot(log(N_list),log(-squeeze(c3(2,:,j))),'k.','markersize',20)
%     title(sprintf('t^3-model coefficient, t = %i',t_list(j)),'fontsize',16)
