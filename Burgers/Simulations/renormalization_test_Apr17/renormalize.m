function [c1,c2,c3,c4] = renormalize(N_list,u_list,t_list,window_width,time)

markov_energy = zeros(size(u_list));
tmodel_energy = zeros(size(u_list));
t2model_energy = zeros(size(u_list));
t3model_energy = zeros(size(u_list));
t4model_energy = zeros(size(u_list));
...
for i = 1:length(N_list)
    ...
    for j = 1:length(t_list)
        ...
        if time
            tmodel_energy(1:N,i,j) = tmodel_energy(:,j)*t;
        end
        ...
        t4model_energy(:,j) = t4(1:N).*conj(u) + conj(t4(1:N)).*u;
        if time
...
for j = 1:num_windows
    ...
    for i = 1:length(N_list)
        ...
        N = N_list(i);
        exact = exact_derivative(1:N,i,window);
        R0 = markov_energy(1:N,i,window);
        R1 = tmodel_energy(1:N,i,window);
        R2 = t2model_energy(1:N,i,window);
        R3 = t3model_energy(1:N,i,window);
        R4 = t4model_energy(1:N,i,window);
