function [nonlin3,uk6,E,E_star,F,F_star] = t3model_term_complete(alpha,F_modes,G_modes,k,epsilon,u_full,uu,uu_star,uk3,A,A_star,B,B_star,C,C_star,D_star)
%
%Computes the complete t^3-model term for a given state vector
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Inputs:
...
E = convolution_sum(1j*epsilon^2*uk3+uu,1j*epsilon^2*uk3+uu,alpha);
E_star = E;
E(G_modes) = 0;
E_star(F_modes) = 0;

F = convolution_sum(uu_star,1j*epsilon^2*uk3+uu,alpha);
F_star = F;
F(G_modes) = 0;
F_star(F_modes) = 0;
...
int2 = convolution_sum(u_full,-epsilon^4.*uk6+1j*epsilon^2.*(A+A_star)...
...
nonlin3 = 2*convolution_sum(u_full,-k.^3*epsilon^4.*A_star + 2*1j*epsilon^2*k.^3.*int1...
                            + 2*int2 + 2*int3 + 2*int4,alpha)...
          +6*convolution_sum(uu_star,1j*epsilon^2.*A_star + 2*int5,alpha);