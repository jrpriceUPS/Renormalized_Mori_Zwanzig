function nonlin=renormalized_complete_3rd_order_fixed_coeff(u,t,simulation_params)
%
...
[nonlin2,uk3,uu,A,A_star,B,B_star,C,C_star,~,D_star] = t2model_term_complete(u_full,nonlin0,uu_star,alpha,F_modes,G_modes,k,epsilon);

%compute t^3-model term
[nonlin3,~,~,~,~,~] = t3model_term_complete(alpha,F_modes,G_modes,k,epsilon,u_full,uu,uu_star,uk3,A,A_star,B,B_star,C,C_star,D_star);

%compute nonlinear part of right hand side of t-model for KdV
nonlin = nonlin0(1:N) + coeffs(1)*nonlin1(1:N) + coeffs(2)*nonlin2(1:N) + coeffs(3)*nonlin3(1:N);