function [nonlin2,uk3,uu,A,A_star,B,B_star,C,C_star,D,D_star] = t2model_term_complete(u_full,nonlin0,uu_star,alpha,F_modes,G_modes,k,epsilon)
%
%Computes the complete t^2-model term for a given state vector
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Inputs:
...
B = convolution_sum(1j*epsilon^2*uk3+uu,u_full,alpha);
B_star = B;
B(G_modes) = 0;
B_star(F_modes) = 0;

C = convolution_sum(uu_star,u_full,alpha);
C_star = C;
C(G_modes) = 0;
C_star(F_modes) = 0;

D = convolution_sum(uu_star,uu_star,alpha);
D_star = D;
D(G_modes) = 0;
D_star(F_modes) = 0;

nonlin2 =  -2*convolution_sum(u_full,1j*epsilon^2*A_star - 2*B_star + 2*C_star,alpha) - 2*D;