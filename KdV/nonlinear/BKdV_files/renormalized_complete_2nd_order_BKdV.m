function nonlin=renormalized_complete_2nd_order_BKdV(u,t,simulation_params)
%
%Computes the nonlinear part of the right hand side of the complete 
%t^2-model of the Burgers-KdV equation based upon a "full" model with M positive 
...
%compute Markov term
    [nonlin0,u_full] = markov_term_BKdV(u,M,N);

%compute t-model term
    [nonlin1,uu_star] = tmodel_term_BKdV(u_full,nonlin0,F_modes);

%compute t^2-model term
    [nonlin2,~,~,~,~,~,~,~,~,~] = t2model_term_complete_BKdV(u_full,nonlin0,uu_star,F_modes,G_modes,k,epsilon);
