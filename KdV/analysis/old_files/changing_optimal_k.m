function changing_optimal_k(t_list,u_list,simulation_params,width,spacing,N_list)
...
coeffs_list = k_dependent_coefficients2(t_list,energy_flow_list,nonlin0_energy_flow,nonlin1_energy_flow,nonlin2_energy_flow,nonlin3_energy_flow,nonlin4_energy_flow,N_list,width,spacing,0);
coeffs_array = no_k_dependence_coeffs(t_list,energy_flow_list,nonlin0_energy_flow,nonlin1_energy_flow,nonlin2_energy_flow,nonlin3_energy_flow,nonlin4_energy_flow,N_list,0,10,0);
...
for k = 1:9;
    ...
    for i = 1:(max(t_list)-width)/spacing+1
        ...
        subplot(2,2,3)
        hold off
        plot(1:N_list(k)-1,coeffs_list(k,2*(N_list(k)-1)+1:3*(N_list(k)-1),i))
        hold on
        plot(1:N_list(k)-1,coeffs(3)*ones(1,N_list(k)-1),'r')
        title(sprintf('N = %i',N_list(k)),'fontsize',16)
        
        subplot(2,2,4)
        hold off
        plot(1:N_list(k)-1,coeffs_list(k,3*(N_list(k)-1)+1:4*(N_list(k)-1),i))
        hold on
        plot(1:N_list(k)-1,coeffs(4)*ones(1,N_list(k)-1),'r')
        title(sprintf('N = %i',N_list(k)),'fontsize',16)
