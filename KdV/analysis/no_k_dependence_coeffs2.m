function coeffs_list = no_k_dependence_coeffs2(t_list,energy_flow_list,nonlin0_energy_flow,nonlin1_energy_flow,nonlin2_energy_flow,N_list,window_width,spacing,disp_plots)
%
%Calculates optimal 2nd order ROM coefficients for fitting the net energy
...
for j = 1:length(window_edges)
    ...
    for i = 1:length(N_list)
        ...
        A2 = [sum(sum(x2.*x2)) sum(sum(x2.*y2))
              sum(sum(y2.*x2)) sum(sum(y2.*y2))];
        ...
        if disp_plots == 1
            ...
            plot(t_list(range),r)
            hold on
            plot(t_list(range),coeffs(1)*x + coeffs(2)*y,'r')
            current_ax = axis;
            ...
            for k = 1:N-1
                ...
                hold on
                plot(t_list(range),coeffs(1)*x2(k+1,:)+coeffs(2)*y2(k+1,:),'r')
