function [t_list,u_list]=renormalized_4_new2(N,epsilon,alpha,endtime,dt,howoften,tol1,tol2,blowup)
%
...
for i=1:nsteps
    ...
    if jettison == 0
        for j = 1:4
            if done_yet(j) ~= 1
                N0 = N_list(j);
                [nonlinear_markov(j),nonlinear_terms(j,:)] = renormalized_3rd_order_energy(u(1:N0),2*N0+1,N0,epsilon,alpha,F_modes{j},G_modes{j},k{j},t(i));
        ...
        if sum(done_yet) == 4
            ...
            B = @(x,t) renormalized_3rd_order(x,2*N+1,N,epsilon,alpha,F_modes{1},G_modes{1},k{1},t,coeffs);
    ...
    if (sum(2*abs(u).^2)-total_energy)/total_energy>tol1
        if blowup
