function coeffs_list = phase_coeffs_only_t1_and_t2(t_list,u_deriv_list,nonlin0_deriv,nonlin1_deriv,nonlin2_deriv,N_list,window_width,spacing,disp_plots)
%
...
%initialize output
coeffs_list = zeros(length(N_list),2,length(window_edges));
...
for j = 1:length(window_edges)
    ...
    for i = 1:length(N_list)
        ...
        y_complex = squeeze(nonlin1_deriv(i,1:N,range));
        ...
        A1 = [sum(sum(y.*y)) sum(sum(y.*w))
             sum(sum(w.*y)) sum(sum(w.*w))];
          
        b1 = [sum(sum(y.*r));sum(sum(w.*r))];
        
        y = imag(y_complex);
        w = imag(w_complex);
        r = imag(r_complex);
        
        A2 = [sum(sum(y.*y)) sum(sum(y.*w))
             sum(sum(w.*y)) sum(sum(w.*w))];
          
        b2 = [sum(sum(y.*r));sum(sum(w.*r))];
        ...
        if disp_plots == 1
            ...
            hold on
            plot(t_list(range),sum(coeffs(1)*real(y_complex) + coeffs(2)*real(w_complex),1),'r')
            current_ax = axis;
            ...
            hold on
            plot(t_list(range),sum(coeffs(1)*imag(y_complex) + coeffs(2)*imag(w_complex),1),'r')
            current_ax = axis;
            axis([min(t_list(range)),max(t_list(range)),current_ax(3),current_ax(4)]);
            ...
            for k = 1:N-1
                ...
                hold on
                plot(t_list(range),real(coeffs(1)*y_complex(k+1,:)+coeffs(2)*w_complex(k+1,:)),'r')
            ...
            for k = 1:N-1
                ...
                hold on
                plot(t_list(range),imag(coeffs(1)*y_complex(k+1,:)+coeffs(2)*w_complex(k+1,:)),'r')
                title(sprintf('N = %i',k+1))
            end
