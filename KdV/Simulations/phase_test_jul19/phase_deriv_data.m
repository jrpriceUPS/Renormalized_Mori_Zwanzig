function [u_deriv_list,nonlin0_deriv,nonlin1_deriv,nonlin2_deriv,nonlin3_deriv,nonlin4_deriv] = phase_deriv_data(t_list,u_list,simulation_params,N_list)
%
%Given a simulation, computes the exact mode derivative at all times.
...
%
%  nonlin0_deriv  =  number of resolutions x max_N x timesteps array
%                    giving the R_0 contribution (Markov) to the 
%                    derivative of each mode at each timestep
%                    for each ROM (given the exact solution)
%
%  nonlin1_deriv  =  number of resolutions x max_N x timesteps array
%                    giving the R_1 contribution to the
%                    derivative of each mode at each timestep for 
%                    each ROM (given the exact solution)
%
%  nonlin2_deriv  =  number of resolutions x max_N x timesteps array
%                    giving the R_2 contribution to the
%                    derivative of each mode at each timestep for 
%                    each ROM (given the exact solution)
%
%  nonlin3_deriv  =  number of resolutions x max_N x timesteps array
%                    giving the R_3 contribution to the
%                    derivative of each mode at each timestep for 
%                    each ROM (given the exact solution)
%
%  nonlin4_deriv  =  number of resolutions x max_N x timesteps array
%                    giving the R_4 contribution to the
%                    derivative of each mode at each timestep for 
%                    each ROM (given the exact solution)
...
%initialize output
nonlin0_deriv = zeros(length(N_list),N,timesteps);
nonlin1_deriv = zeros(length(N_list),N,timesteps);
nonlin2_deriv = zeros(length(N_list),N,timesteps);
nonlin3_deriv = zeros(length(N_list),N,timesteps);
nonlin4_deriv = zeros(length(N_list),N,timesteps);
...
for j = 1:length(N_list)
    for i = 1:timesteps
        ...
        nonlin0_deriv(j,1:N0,i) = nonlin0(1:N0);
        nonlin1_deriv(j,1:N0,i) = nonlin1(1:N0);
        nonlin2_deriv(j,1:N0,i) = nonlin2(1:N0);
        nonlin3_deriv(j,1:N0,i) = nonlin3(1:N0);
        nonlin4_deriv(j,1:N0,i) = nonlin4(1:N0);
