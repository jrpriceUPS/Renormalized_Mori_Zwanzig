function coeffs_list = t2_and_t4_coeffs(t_list,energy_flow_list,nonlin0_energy_flow,nonlin1_energy_flow,nonlin2_energy_flow,nonlin3_energy_flow,nonlin4_energy_flow,N_list,window_width,spacing,disp_plots)
%
%Calculates optimal t2 and t4 ROM coefficients for fitting the net energy
...
for j = 1:length(window_edges)
    ...
    for i = 1:length(N_list)
        ...
        A2 = [sum(sum(y2.*y2)) sum(sum(y2.*w2))
              sum(sum(w2.*y2)) sum(sum(w2.*w2))];
          
        b2 = [sum(sum(y2.*r2));sum(sum(w2.*r2))];
        ...
        if disp_plots == 1
            ...
            plot(t_list(range),r)
            hold on
            plot(t_list(range),coeffs(2)*y + coeffs(4)*w,'r')
            current_ax = axis;
            ...
            for k = 1:N-1
                ...
                hold on
                plot(t_list(range),coeffs(2)*y2(k+1,:)+coeffs(4)*w2(k+1,:),'r')
